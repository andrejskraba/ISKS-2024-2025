...
function pošljiSporočilo(){
    let sporočilo = document.getElementById("sporočilo").value; // preberemo vrednost iz vnosnega polja
    povezava.send(sporočilo); // na strežnik (esp32) pošljemo sporočilo preko WebSocket-a
}
</script>
</body>
</html>
  ...
  case WStype_TEXT:
    {
      ...
      String besedilo = String((char) payload[0]); // prvi znak v spremenljivki payload tipa char pretvorimo v strig

      if(besedilo == "1"){
        digitalWrite(LED1pin, 1); // na nožici (pinu) št. 2 je vgrajena LED dioda, zapis visoke vrednosti (1=HIGH)
      }
      if(besedilo == "0"){
        digitalWrite(LED1pin, 0); // na nožici 2 zapišemo nizko vrednost
      }
