...
void handle_root(){ // če bo strežnik dobil zahtevo, t.j. "request"
  server.send(200, "text/html", "Pozdravljen svet iz esp32!"); // bo vrnil odgovor ("response") "Pozdravljen ..."
  // 200 predstavlja kodo za OK oz. vse je v redu
  // kot odgovor vrnemo niz oz. "String" "Pozdravljen svet iz esp32!"
}

void handle_abeceda(){ // če bo strežnik dobil zahtevo, t.j. "request" "/abeceda"
  server.send(200, "text/html; charset=UTF-8", "abcčdefghijklmnoprsštuvzž"); // bo vrnil odgovor ("response") abecedo
}

void handle_cifre(){ // če bo strežnik dobil zahtevo, t.j. "request" "/abeceda"
  server.send(200, "text/html", "0123456789"); // bo vrnil odgovor ("response") 0123456789
}
...
void setup() {
  ...
  server.on("/abeceda", handle_abeceda); // tu povemo, kaj naj se dogodi, če kot zahtevo prejmemo poševnico "/"
  server.on("/cifre", handle_cifre); // tu povemo, kaj naj se dogodi, če kot zahtevo prejmemo poševnico "/"
